#!/bin/bash
#
#  Web Launcher and working example for Weaved Direct Connections
#   (MAC only)
#
#  wweb <-v> <-v> weavedsshdevicename
#
...
WWEB - A web launcer connection wrapper for Weaved
--------------------------------------------------
This software allows you to make a direct connection to your Weaved enabled web servers.
...
To make an web connection to any given device, use:
...
./wweb.sh -v device-name

./wweb.sh -v -v device-name
...
#
echo "Weaved wweb.sh Version $VERSION $MODIFIED"
create_config
...
if [  -e $WEAVED_DIR/$port.active ]; then
    ...
    if [ $VERBOSE -gt 0 ]; then
        ...
        echo "Running command>> open -a \"Safari\" \"127.0.0.1:$port"
    fi
